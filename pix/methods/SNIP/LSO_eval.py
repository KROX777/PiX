        ...
        for problem_name in problem_names:
            ...
            try:
                ...
                while (done_bagging == False) and (bag_number <= max_bags):
                    ...
                    # Snapshot batch_results for evaluation/saving
                    ...
                    # Pretty-print the predicted tree in a human-friendly infix form.
                    try:
                        tree_item = (
                            final_results["direct_predicted_tree"].iloc[0]
                            if hasattr(final_results["direct_predicted_tree"], "iloc")
                            else final_results["direct_predicted_tree"][0]
                        )

                        if hasattr(tree_item, "infix"):
                            human_tree = tree_item.infix()
                        else:
                            human_tree = str(tree_item)

                        replace_ops = {"add": "+", "mul": "*", "sub": "-", "pow": "**", "inv": "1/"}
                        for old, new in replace_ops.items():
                            human_tree = human_tree.replace(old, new)

                        print("Tree (human):", human_tree)
                    except Exception:
                        try:
