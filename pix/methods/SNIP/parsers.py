...
def get_parser():
    ...
    # main parameters
    parser.add_argument(
        "--lso_optimizer", type=str, default="gwo", help="gwo,cma-es,ngopt,twopoints-de,randomsearch"
    )
    ...
    parser.add_argument(
        "--lso_pop_size", type=int, default=30, help="Population size for LSO"
    )

    parser.add_argument(
        "--lso_stop_r2", type=float, default=0.99, help="Stop criterion for LSO"
    )
    ...
    parser.add_argument(
        "--bt_lambda", type=float, default=0.0051, help="Barlow Twins Lambda in Loss"
    )
    ...
    parser.add_argument(
        "--reload_model_snipenc", type=str, default="./weights/snip-10dmax.pth", help="Reload a pretrained snip encoder for symbolic regression"
    )
    parser.add_argument(
        "--reload_model_e2edec", type=str, default="./weights/e2e.pth", help="Reload a pretrained e2e decoder for symbolic regression"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--contrastive_weight", type=float, default=1.0, help="The coefficient for contrastive loss")
