...
class SNIPLSORegressor(BaseEstimator):
    def __init__(self, trainer,
        ...
        self.model = SNIPSymbolicRegressor(params = params, env=self.env, modules=trainer.modules)
    ...
    def fit(self, X_scaled_to_fit, Y_scaled_to_fit, x_to_fit, y_to_fit, x_to_predict, y_to_predict, formula="", problem_name="", max_bags=1):
        if self.rescale:
            raise NotImplementedError("Rescale not implemented in fit function yet.")
        
        bag_number = 1
        self.batch_results = defaultdict(list)
        max_r2_zero = 0
        
        while bag_number <= max_bags:
            bag_number += 1
            ...
            with torch.no_grad():
                if self.params.lso_optimizer == "gwo":
                    lso_out = lso_fit(sample_to_learn, self.env, self.params, self.model, self.batch_results, bag_number)
                else:
                    opt_LSO = LSOFitNeverGrad(self.env, self.params, self.model, sample_to_learn, self.batch_results, bag_number)
                    lso_out = opt_LSO.fit_func()

            # Create a temporary DataFrame snapshot for evaluation and saving,
            # but keep `self.batch_results` as the original defaultdict(list)
            df = pd.DataFrame.from_dict(self.batch_results)
            df.insert(0, "problem", problem_name)
            df.insert(0, "formula", formula)
            df["input_dimension"] = x_to_fit.shape[1]
            df["bag_number"] = bag_number
            if df["r2_zero_final_fit"][0] > max_r2_zero:
                final_results = df.copy()
                max_r2_zero = df["r2_zero_final_fit"][0]

        print("R2 zero final fit: ", final_results["r2_zero_final_fit"][0])
        ...
        #     if self.first_write:
        #         final_results.to_csv(self.save_file, index=False)
        #         self.first_write = False
        #     else:
        #         final_results.to_csv(
        #             self.save_file, mode="a", header=False, index=False
        #         )
        print("Tree: " + final_results["direct_predicted_tree"][0])
    
    def __str__(self):
        return "SNIPLSORegressor"
...
def evaluate_pmlb_lso_with_regressor(
        trainer,
        params,
        target_noise=0.0,
        random_state=29910,
        ...
        all_datasets = pd.read_csv(
            "/sfs/xcy/srdata/pmlb/pmlb/all_summary_stats.tsv",
            sep="\t",)
        ...
        pmlb_path = "/sfs/xcy/srdata/pmlb/datasets/"  # high_dim_datasets

        feynman_problems = pd.read_csv(
            "/sfs/xcy/srdata/feynman/feynman/FeynmanEquations.csv",
            delimiter=",",)
        ...
        for problem_name in problem_names:
            counter += 1
            if counter == 1:        # TODO: delete this test line
                continue
            ...
            try:
                ...
                regressor.fit(X_scaled_to_fit, Y_scaled_to_fit, x_to_fit, y_to_fit, x_to_predict, y_to_predict, formula=formula, problem_name=problem_name, max_bags=max_bags)
...
if __name__ == '__main__':
    ...
    if params.eval_lso_on_pmlb:
        ...
        evaluate_pmlb_lso_with_regressor(
            trainer,
            params,
            target_noise=target_noise,
